...
function s(){
	...
	window.showModalDialog("examQuestionSelectInit.action?commonSeq=" + key, window, popstyle);
}
function t(){
	var key = document.getElementById("commonSeq").value;
...
function d(){
	var key = document.getElementById("commonSeq").value;
	var popstyle="dialogTop:80px;dialogLeft:650px;help:no;center:yes;dialogHeight:700px;dialogWidth:800px;status:no;resizable:yes;scroll:no;loacation:no;toolbar:no;";
	window.showModalDialog("departmentSelectInit.action?pageType=performance&commonSeq=" + key, window, popstyle);
}
/* function e(){
	var key = document.getElementById("commonSeq").value;
...
function f(){
	...
	$("#itemsInfoDiv").load("examPlanItemInfoRefresh.action",param);
}
function Del(id){
	var key = document.getElementById("commonSeq").value;
    var param = {"param1" : id,"commonSeq" : key};
	$("#itemsInfoDiv").load("examPlanItemInfoDelete.action",param);
}
function e(){
 	var key = document.getElementById("commonSeq").value;
    var param = {"commonSeq" : key};
	$("#employeeInfoDiv").load("examPlanEmpInfoRefresh.action",param);
}
function DelEmp(id){
	var key = document.getElementById("commonSeq").value;
    var param = {"param1" : id,"commonSeq" : key};
    $("#employeeInfoDiv").load("examPlanEmpInfoDelete.action",param);
}
...
<body>
<s:form action="examPlanInfoSave" method="post" theme="simple">
...
<!--  	        <s:property value="errMsg"/> -->
    ...
    <tr>
        ...
        <td>
			...
			<div id="tabLabel__2" onclick='tabClick(2,3)' style="float:left;padding-left:10px;padding-right:10px;background-image: url(images/menu/tab_unactive_bg.gif);">Employee Information</div>
...
<div id='tabContent__0' style='display:block;'>
	<table cellpadding="2" cellspacing="1" border="0" width="100%" align="center" style="margin-top:5px;">
		<tr>
			...
			<td	width="74%"	class=td1>
			    <input name="mp7002.MP7002_TITLE" value="${mp7002.MP7002_TITLE}" type="text" style="width:80%;"/>  (Must Fill)
		        <s:fielderror theme="simple" cssStyle="color:red;margin-left:10px;">
		            <s:param>mp7002.MP7002_TITLE</s:param>
		        </s:fielderror>
			</td>
		</tr>
		<tr>
			<td	width="26%"	align="right" class=td1>Year:</td>
			<td	width="74%"	class=td1><input name="mp7002.MP7002_YEAR" value="${mp7002.MP7002_YEAR}" type="text" style="width:80%;"/></td>
		</tr>
		<tr>
			<td	width="26%"	align="right" class=td1>Month:
			<td	width="74%"	class=td1><s:select id="param4" name="param4" value="param4" list="commonMonthList" theme="simple" style="width:80%;"/></td>
		</tr>
		<tr>
			<td	width="26%"	align="right" class=td1>Plan Type:
			<td	width="74%"	class=td1>
			    <input type="radio" name="mp7002.MP7002_TYPE" value="1" <s:if test="mp7002.MP7002_TYPE == 1">checked</s:if> style="border:none;">Yearly</input>
			    <input type="radio" name="mp7002.MP7002_TYPE" value="2" <s:if test="mp7002.MP7002_TYPE == 2">checked</s:if> style="border:none;">Monthly</input>
			</td>
		</tr>
	<s:if test="pageType == 'edit'">
		...
		<tr>
			...
			<td	width="74%"	class=td1><s:textarea name="mp7002.MP7002_COMMENT" style="width:80%;height:60px;" theme="simple"></s:textarea></td>
		</tr>
...
<div id='tabContent__1' style='display:none;height:500px;overflow-y:scroll;'>
	<div style="width:100%;float:left;margin-top:10px;text-align:center;">
	    <input type="button" class="input0" name="addQuestion" onclick="s()" value="Add Appraisal Questionnaire" <s:if test="optSave == 0 || pageType != 'edit' " >disabled="disabled"</s:if>/>
	    <input type="submit" class="input0" name="Submit2" value="Save" <s:if test="optSave == 0" >disabled="disabled"</s:if>/>
	</div>
	...
	<s:iterator id="list" value="examItemsInfo" status="st">
	    <tr align="left" style="height:28px;">
	        ...
	        <td width="50px" align="center">
	            <input type="button" onclick="Del('<s:property value="MP7003_SEQ"/>')" name="delBtn" value="Del" id="delBtn" />
	        </td>
	    </tr>
	</s:iterator>
	</table>
...
<div id='tabContent__2' style='display:none;height:500px;overflow-y:scroll;'>
	<div style="width:100%;float:left;margin-top:10px;text-align:center;">
	    <input type="button" class="input0" name="addQuestion" onclick="d()" value="Select Department" <s:if test="optSave == 0 || pageType != 'edit'" >disabled="disabled"</s:if>/>
	    <input type="button" class="input0" name="addQuestion" onclick="t()" value="Select Employee" <s:if test="optSave == 0 || pageType != 'edit'" >disabled="disabled"</s:if>/>
	    <input type="submit" class="input0" name="Submit3" value="Save" <s:if test="optSave == 0" >disabled="disabled"</s:if>/>
	</div>

	<div id="employeeInfoDiv" style="width:100%;float:left;height:550;overflow:auto;">
	<table cellspacing="0" cellpadding="0" style="border:0;width:100%;">
	    <tr align="center" class="tdBg">
	        <th scope="col" class="tdBg">Seq</th>
