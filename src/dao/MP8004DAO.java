...
public class MP8004DAO extends HibernateDaoSupport implements IMP8004DAO{
    // 保存数据
    public void save(MP8004 mp8004) {
        if(mp8004 != null){
            getHibernateTemplate().save(mp8004);
        }
    }
    // 删除数据
    public void delete(MP8004 mp8004) {
        if(mp8004 != null){
            getHibernateTemplate().delete(mp8004);
        }
    }
    // 根据KEY检索数据
    public MP8004 findById(String key) {
        	return (MP8004)getHibernateTemplate().get("entity.MP8004", key);
    }
    // 取得所有有效数据
    ...
    public void update(MP8004 mp8004) {
        if(mp8004 != null){
            getHibernateTemplate().update(mp8004);
        }
    }
    // 更新数据
