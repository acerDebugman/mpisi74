...
public class MP8004DAO   implements IMP8004DAO{
    ...
    public void save(MP8004 mp8004) {
        if(mp8004 != null){
            sf.getCurrentSession().save(mp8004);
        }
    }
    // 删除数据
    public void delete(MP8004 mp8004) {
        if(mp8004 != null){
            sf.getCurrentSession().delete(mp8004);
        }
    }
    // 根据KEY检索数据
    public MP8004 findById(String key) {
        	return (MP8004)sf.getCurrentSession().get("entity.MP8004", key);
    }
    // 取得所有有效数据
    @SuppressWarnings("unchecked")
    public List<MP8004> findAll() {
        return sf.getCurrentSession().createQuery(" from MP8004 where 1=1  ").list();
    }
    // 更新数据
    public void update(MP8004 mp8004) {
        if(mp8004 != null){
            sf.getCurrentSession().update(mp8004);
        }
    }
    // 更新数据
