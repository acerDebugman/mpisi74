...
public class MP2007DAO   implements IMP2007DAO{
	...
	public void save(MP2007 mp2007) {
		if(mp2007 != null){
			sf.getCurrentSession().save(mp2007);
		}
	}
	// 删除数据
	public void delete(MP2007 mp2007) {
		if(mp2007 != null){
			sf.getCurrentSession().delete(mp2007);
		}
	}
	// 根据KEY检索数据
	public MP2007 findById(int key) {
		MP2007 mp2007 = (MP2007)sf.getCurrentSession().get("entity.MP2007", key);
	...
	public List<MP2007> findAll() {
		return sf.getCurrentSession().createQuery(" from MP2007 where 1=1 ").list();
	}
	// 更新数据
	public void update(MP2007 mp2007) {
		if(mp2007 != null){
			sf.getCurrentSession().update(mp2007);
		}
	}
	// 动态根据传入的参数，检索数据
