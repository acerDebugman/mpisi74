...
public class MP8003DAO   implements IMP8003DAO{
    ...
    public void save(MP8003 mp8003) {
        if(mp8003 != null){
            sf.getCurrentSession().save(mp8003);
        }
    }
    // 删除数据
    public void delete(MP8003 mp8003) {
        if(mp8003 != null){
            sf.getCurrentSession().delete(mp8003);
        }
    }
    // 根据KEY检索数据
    public MP8003 findById(String key) {
        	return (MP8003)sf.getCurrentSession().get("entity.MP8003", key);
    }
    // 取得所有有效数据
    @SuppressWarnings("unchecked")
    public List<MP8003> findAll() {
        return sf.getCurrentSession().createQuery(" from MP8003 where 1=1  ").list();
    }
    // 更新数据
    public void update(MP8003 mp8003) {
        if(mp8003 != null){
            sf.getCurrentSession().update(mp8003);
        }
    }
    // 更新数据
