...
public class MP7003DAO   implements IMP7003DAO{
	...
	public void save(MP7003 mp7003) {
		if(mp7003 != null){
			sf.getCurrentSession().save(mp7003);
		}
	}
	// 删除数据
	public void delete(MP7003 mp7003) {
		if(mp7003 != null){
			sf.getCurrentSession().delete(mp7003);
		}
	}
	// 根据KEY检索数据
	public MP7003 findById(String key) {
		return (MP7003)sf.getCurrentSession().get("entity.MP7003", key);
	}
	// 取得所有有效数据
	@SuppressWarnings("unchecked")
	public List<MP7003> findAll() {
		return sf.getCurrentSession().createQuery(" from MP7003 where 1=1 and MP7003_STATUS = '1' ").list();
	}
	// 更新数据
	public void update(MP7003 mp7003) {
		if(mp7003 != null){
			sf.getCurrentSession().update(mp7003);
		}
	}
	// 动态根据传入的参数，检索数据
