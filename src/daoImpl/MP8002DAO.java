...
public class MP8002DAO   implements IMP8002DAO{
    ...
    public void save(MP8002 mp8002) {
        if(mp8002 != null){
            sf.getCurrentSession().save(mp8002);
        }
    }
    // 删除数据
    public void delete(MP8002 mp8002) {
        if(mp8002 != null){
            sf.getCurrentSession().delete(mp8002);
        }
    }
    // 根据KEY检索数据
    public MP8002 findById(String key) {
        	return (MP8002)sf.getCurrentSession().get("entity.MP8002", key);
    }
    // 取得所有有效数据
    @SuppressWarnings("unchecked")
    public List<MP8002> findAll() {
        return sf.getCurrentSession().createQuery(" from MP8002 where 1=1 and MP8002_STATUS = '1' ").list();
    }
    // 更新数据
    public void update(MP8002 mp8002) {
        if(mp8002 != null){
            sf.getCurrentSession().update(mp8002);
        }
    }
    // 更新数据
