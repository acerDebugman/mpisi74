...
public class MP7007DAO   implements IMP7007DAO{
	...
	public void save(MP7007 mp7007) {
		if(mp7007 != null){
			sf.getCurrentSession().save(mp7007);
		}
	}
	// 删除数据
	public void delete(MP7007 mp7007) {
		if(mp7007 != null){
			sf.getCurrentSession().delete(mp7007);
		}
	}
	// 根据KEY检索数据
	public MP7007 findById(String key) {
		return (MP7007)sf.getCurrentSession().get("entity.MP7007", key);
	}
	// 取得所有有效数据
	@SuppressWarnings("unchecked")
	public List<MP7007> findAll() {
		return sf.getCurrentSession().createQuery(" from MP7007 where 1=1 and MP7007_STATUS = '1' ").list();
	}
	// 更新数据
	public void update(MP7007 mp7007) {
		if(mp7007 != null){
			sf.getCurrentSession().update(mp7007);
		}
	}
	// 动态根据传入的参数，检索数据
