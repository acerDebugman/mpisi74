...
public class MP8006DAO   implements IMP8006DAO{
    ...
    public void save(MP8006 mp8006) {
        if(mp8006 != null){
            sf.getCurrentSession().save(mp8006);
        }
    }
    // 删除数据
    public void delete(MP8006 mp8006) {
        if(mp8006 != null){
            sf.getCurrentSession().delete(mp8006);
        }
    }
    // 根据KEY检索数据
    public MP8006 findById(String key) {
        	return (MP8006)sf.getCurrentSession().get("entity.MP8006", key);
    }
    // 取得所有有效数据
    @SuppressWarnings("unchecked")
    public List<MP8006> findAll() {
        return sf.getCurrentSession().createQuery(" from MP8006 where 1=1  ").list();
    }
    // 更新数据
    public void update(MP8006 mp8006) {
        if(mp8006 != null){
            sf.getCurrentSession().update(mp8006);
        }
    }
	// 更新数据
