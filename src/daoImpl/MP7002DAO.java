...
public class MP7002DAO   implements IMP7002DAO{
	...
	public void save(MP7002 mp7002) {
		if(mp7002 != null){
			sf.getCurrentSession().save(mp7002);
		}
	}
	// 删除数据
	public void delete(MP7002 mp7002) {
		if(mp7002 != null){
			sf.getCurrentSession().delete(mp7002);
		}
	}
	// 根据KEY检索数据
	public MP7002 findById(String key) {
		return (MP7002)sf.getCurrentSession().get("entity.MP7002", key);
	}
	// 取得所有有效数据
	@SuppressWarnings("unchecked")
	public List<MP7002> findAll() {
		return sf.getCurrentSession().createQuery(" from MP7002 where 1=1 and MP7002_STATUS = '1' ").list();
	}
	// 更新数据
	public void update(MP7002 mp7002) {
		if(mp7002 != null){
			sf.getCurrentSession().update(mp7002);
		}
	}
	// 动态根据传入的参数，检索数据
