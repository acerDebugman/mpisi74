...
public class MP2009DAO   implements IMP2009DAO{
    ...
    public void save(MP2009 mp2009) {
        if(mp2009 != null){
            sf.getCurrentSession().save(mp2009);
        }
    }
    // 删除数据
    public void delete(MP2009 mp2009) {
        if(mp2009 != null){
            sf.getCurrentSession().delete(mp2009);
        }
    }
    // 根据KEY检索数据
    public MP2009 findById(String key) {
        	return (MP2009)sf.getCurrentSession().get("entity.MP2009", key);
    }
    // 取得所有有效数据
    @SuppressWarnings("unchecked")
    public List<MP2009> findAll() {
        return sf.getCurrentSession().createQuery(" from MP2009 where 1=1  ").list();
    }
    // 更新数据
    public void update(MP2009 mp2009) {
        if(mp2009 != null){
            sf.getCurrentSession().update(mp2009);
        }
    }
    // 更新数据
