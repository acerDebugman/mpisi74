...
public class MP7005DAO   implements IMP7005DAO{
	...
	public void save(MP7005 mp7005) {
		if(mp7005 != null){
			sf.getCurrentSession().save(mp7005);
		}
	}
	// 删除数据
	public void delete(MP7005 mp7005) {
		if(mp7005 != null){
			sf.getCurrentSession().delete(mp7005);
		}
	}
	// 根据KEY检索数据
	public MP7005 findById(String key) {
		return (MP7005)sf.getCurrentSession().get("entity.MP7005", key);
	}
	// 取得所有有效数据
	@SuppressWarnings("unchecked")
	public List<MP7005> findAll() {
		return sf.getCurrentSession().createQuery(" from MP7005 where 1=1 and MP7005_STATUS = '1' ").list();
	}
	// 更新数据
	public void update(MP7005 mp7005) {
		if(mp7005 != null){
			sf.getCurrentSession().update(mp7005);
		}
	}
	// 更新数据
