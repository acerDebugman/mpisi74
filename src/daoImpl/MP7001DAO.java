...
public class MP7001DAO   implements IMP7001DAO{
	...
	public void save(MP7001 mp7001) {
		if(mp7001 != null){
			sf.getCurrentSession().save(mp7001);
		}
	}
	// 删除数据
	public void delete(MP7001 mp7001) {
		if(mp7001 != null){
			sf.getCurrentSession().delete(mp7001);
		}
	}
	// 根据KEY检索数据
	public MP7001 findById(String key) {
		return (MP7001)sf.getCurrentSession().get("entity.MP7001", key);
	}
	// 取得所有有效数据
	@SuppressWarnings("unchecked")
	public List<MP7001> findAll() {
		return sf.getCurrentSession().createQuery(" from MP7001 where 1=1 and MP7001_STATUS = '1' ").list();
	}
	// 更新数据
	public void update(MP7001 mp7001) {
		if(mp7001 != null){
			sf.getCurrentSession().update(mp7001);
		}
	}
	// 动态根据传入的参数，检索数据
